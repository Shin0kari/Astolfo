...
int main ()
{
	...
	if (a == 1)
	{
		setvbuf(stdout, NULL, _IONBF, 0);    //Задумка была такая: сделать калькулятор
    	setvbuf(stderr, NULL, _IONBF, 0);    //не нагруженным в использовании, похожим
	...
	else if (a == 2)
	{
		...
		scanf(" %i",&size);                                                  //может и с 1 но я не уверен, не проверял ))
		scanf(" %i",&num);
		printf("\n");
		printf(" %i  <-num", num);
		a = malloc((size*num)*sizeof(int));
		...
		case '-':
		{
			...
			for (int j=0;j<size;j++)       //суть в том, что вы можете использовать
				res[j]=0;                  //больше 1 вектора в 1 операции
			for (int i=0;i<num;i++)
			{
				for (int j=0;j<size;j++)
				{
